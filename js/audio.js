...
export class AudioEngine {
  ...
  get track() { return this.playlist[this.index] || null; }
  get duration() { return this.el.duration || 0; }
  get time() { return this.el.currentTime || 0; }
  get progress() { return this.duration ? clamp(this.time / this.duration, 0, 1) : 0; }
  ...
  async toggle() {
    ...
    if (this.playing) {
      ...
      try { await this.el.play(); this.playing = true; } catch { this.playing = false; }
    }
    this.onStateChange?.();
    return this.playing;
  }
  ...
  pause() {
    if (this.playing) { this.el.pause(); this.playing = false; this.onStateChange?.(); }
  }
  ...
  skip(dir) {
    ...
    if (keep) { this.ensureCtx(); this.el.play().catch(() => {}); }
    this.sfx('chunk');
    this.onStateChange?.();
  }
  ...
  stopSeek(silent = false) {
    ...
    clearInterval(this._rewTimer); this._rewTimer = null;
    this._stopWhir();
  ...
  _onEnded() {
    ...
    if (this.playing || this._wasPlaying || wasSeeking) {
      this.el.play().then(() => { this.playing = true; this.onStateChange?.(); }).catch(() => {});
    }
  }
  ...
  _stopWhir() {
    ...
    setTimeout(() => { try { src.stop(); lfo.stop(); } catch {} }, 200);
    this._whir = null;
  }

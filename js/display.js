...
const SW = 192, SH = 40, SCALE = 4;   // 小画布尺寸与放大倍数
const AMBER = '#ffb054';
...
export class DotDisplay {
  constructor() {
    ...
    this.big.width = SW * SCALE; this.big.height = SH * SCALE;
    this.bctx = this.big.getContext('2d');

    this.mid = document.createElement('canvas');
    this.mid.width = this.big.width; this.mid.height = this.big.height;
  ...
  _buildDotPattern() {
    ...
    pat.width = cell; pat.height = cell;
  ...
  update(dt, state) {
    ...
    if (msgActive) {
      ...
      const trk = `TRK ${String(state.index + 1).padStart(2, '0')}/${String(Math.max(state.count, 1)).padStart(2, '0')}`;
      c.fillText(trk, 16, 2);
  ...
  _drawStateIcon(c, x, y, state) {
    ...
    c.beginPath();
    if (state.seekDir > 0) {
      c.moveTo(0, 0); c.lineTo(4.5, 3.5); c.lineTo(0, 7);
      c.moveTo(5, 0); c.lineTo(9.5, 3.5); c.lineTo(5, 7);
    } else if (state.seekDir < 0) {
      c.moveTo(4.5, 0); c.lineTo(0, 3.5); c.lineTo(4.5, 7);
      c.moveTo(9.5, 0); c.lineTo(5, 3.5); c.lineTo(9.5, 7);
    } else if (state.playing) {
      c.moveTo(1, 0); c.lineTo(8, 3.5); c.lineTo(1, 7);
    } else {
      c.rect(1, 0, 2.6, 7); c.rect(5.4, 0, 2.6, 7);
    }
    c.fill();
    c.restore();
  }

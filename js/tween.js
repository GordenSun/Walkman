...
export function tween({ from = 0, to = 1, dur = 0.5, delay = 0, ease = Ease.inOutCubic, onUpdate, onComplete }) {
  const item = {
    t: -delay, dur, from, to, ease, onUpdate, onComplete, done: false,
    cancel() { active.delete(item); },
  };
  active.add(item);
  return item;
}
...
export function tweenAsync(opts) {
  return new Promise((resolve) => {
    tween({ ...opts, onComplete: () => { opts.onComplete?.(); resolve(); } });
  });
}
